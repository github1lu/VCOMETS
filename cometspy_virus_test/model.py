...
class model:
    ...
    def load_cobra_model(self, curr_m : cobra.Model, randomtag : bool = False):
        ...
        # This is just a temporary fix to find the objective
        # Ignores the cobra multiple objective and only takes the first one
        obj = [str(x).split(':')[0]
               for x in reaction_list
               if x.objective_coefficient != 0][0]
        
        tmp = self.reactions.loc[self.reactions['REACTION_NAMES'] == obj]['ID']
        self.objectives = {tmp.iloc[0] : 1}
    ...
    def read_comets_model(self, path : str, randomtag : bool = False):
        ...
        # '''----------- OBJECTIVE -----------------------------'''
        #Gives us the line that objective is on (think of it as counting all the new line characters before it)
        lin_obj = re.split('OBJECTIVE',
                           m_filedata_string)[0].count('\n')
        
        num_obj = int(m_f_lines[lin_obj].strip().split()[1])
        
        for i in range(num_obj):
            obj = int(m_f_lines[lin_obj + i + 1].strip().split()[0])
            wt = float(m_f_lines[lin_obj + i + 1].strip().split()[1])
            self.objectives[obj] = wt
    ...
    def write_comets_model(self, working_dir : str = None):
        ...
        with open(path_to_write, 'a') as f:
            ...
            f.write('OBJECTIVE ' + str(len(self.objectives)) + '\n')
            for obj in self.objectives:
                f.write('  ' + str(obj) + ' ' + str(self.objectives[obj]) + '\n')
